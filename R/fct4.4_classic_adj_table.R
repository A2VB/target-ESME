...
fct4.4_classic_adj_table <- function(dd, n_midd=NULL, confounders, arm, vname_weights, label = NULL){


  dd1 <- dd %>%
    select(c(all_of(confounders), !!as.symbol(arm), !!as.symbol(vname_weights)))
  ...
  if(!is.null(label)){
    ...
    tab2 <- tab1 %>%
      survey::svydesign(data = ., ids = ~ 1, weights = ~ get(vname_weights)) %>%
      gtsummary::tbl_svysummary(by=!!as.symbol(arm),
